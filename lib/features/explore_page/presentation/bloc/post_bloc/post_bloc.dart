...
//global variables
  ...
  PostBloc({
    ...
    required GetCurrentUserInformationUsecase getCurrentUserInformationUsecase,
        ...
        _getCurrentUserInformationUsecase = getCurrentUserInformationUsecase,
        super(PostInitial()) {
    ...
    on<GetCurrentUserInformationEvent>(_onGetCurrentUserInformation);
  ...
  void _onMarkStoryViewed(
    ...
    try {
      ...
      final currentViewedStories = Set<String>.from(viewedStories);

      // Add the new story ID to the set
      currentViewedStories.add(event.storyId);

      // Save to local storage immediately
      await _saveViewedStories(currentViewedStories);
      ...
      await res.fold(
        (l) async {
          // Even if remote update fails, keep local changes
          emit(PostStoryViewSuccess(
            viewedStoryId: event.storyId,
            posts: _posts,
            stories: _stories,
            viewedStories: currentViewedStories,
          ));
        },
        (r) async {
          // Emit success state with updated viewed stories
          emit(PostStoryViewSuccess(
            viewedStoryId: event.storyId,
            posts: _posts,
            stories: _stories,
      ...
      emit(PostStoryViewFailure('Failed to mark story as viewed: $e'));
    }
  }
  ...
  void _onGetAllPosts(
    ...
    await res.fold(
      ...
      (r) async {
        ...
        final stories = r
            .where(
                (post) => post.postType == 'Story' && post.category != 'Feeds')
            .toList();
        ...
        await viewedRes.fold(
          ...
          (viewedIds) async {
            if (emit.isDone) return;
            final allViewed = viewedStories..addAll(viewedIds.map((e) => e));
  ...
  void _onReportPost(
    ...
    final res = await _reportPostUseCase(
      ReportPostParams(
        postId: event.postId,
    ...
    await res.fold(
      (l) async {
        if (l.message.contains('already reported')) {
          emit(PostAlreadyReportedState());
        } else {
          emit(PostReportFailure(l.message));
        }
      },
      (r) async => emit(PostReportSuccess()),
    );
  }
  ...
  void _onGetCurrentUserInformation(
    ...
    await result.fold(
      (failure) async =>
          emit(GetCurrentUserInformationFailure(failure.message)),
      (user) async => emit(GetCurrentUserInformationSuccess(user)),
    );
  }
